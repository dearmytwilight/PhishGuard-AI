...
### RestClient vs WebClient 선택 근거

`추가 입력 필요` — FastAPI 서버 호출에 WebClient(Reactive) 대신 RestClient(동기)를 선택한 구체적인 이유를 보강해주시면 좋습니다. 예: "LLM 호출로 인해 AI 서버 응답이 수 초 이상 소요될 수 있는 상황에서 동기 방식을 선택한 기술적 이유는 무엇인가요?"
