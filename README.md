...
> **추가 입력 필요** — 위 내용 외에 개발 과정에서 겪은 구체적인 기술적 문제가 있다면 직접 보완해 주세요. 아래 질문을 참고하세요.
> - 실제 모바일 디바이스에서 테스트 시 `localhost:8000` 하드코딩 API URL로 인해 연결 오류를 겪은 경험이 있나요?
> - 다크 모드 적용 중 특정 컴포넌트에서 색상이 의도대로 반영되지 않은 케이스가 있었나요?
> - Expo New Architecture 적용 후 기존 라이브러리와 호환 문제가 발생한 경우가 있었나요?
